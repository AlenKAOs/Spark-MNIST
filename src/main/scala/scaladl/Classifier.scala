...
object Classifier {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sparkContext
      .hadoopConfiguration.set("fs.s3a.access.key", "<YOUR_ACCESS_KEY>")
    spark.sparkContext
      .hadoopConfiguration.set("fs.s3a.secret.key", "<YOUR_SECRET_KEY>")
    spark.sparkContext
    ...
    // val s3Path = "./dataset/"
    val s3Path = "s3a://streaming-dataset/"
    val train = spark.read.load(s3Path + "mnist_train.parquet")
    val test = spark.read.load(s3Path + "mnist_test.parquet")
    ...
    train.persist(MEMORY_ONLY)
    test.persist(MEMORY_ONLY)
